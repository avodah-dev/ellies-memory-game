...
Timing targets, to be replaced with actual measurements: the prior staging release took about 11 minutes, including a second complete verification. Warm Blacksmith full runs are estimated at 5–8 minutes; retained-image promotions at 1–3 minutes. Cold caches, large image transfers and runner queues may take longer. Before calling this deploy path ready, exercise it through a **real authorized staging push**, verify that it selected a previously verified image and skipped the full gate, then inspect live health and HTTP smoke results. PR CI or a local replay alone does not prove the deploy trigger works.
