...
1. Open a PR and require the existing `verify` check. Blacksmith runs parallel jobs on the same PR head revision: lint/types/unit/server/coverage/rules checks, Vite browser tests, a container image build, and container HTTP/browser tests. Browser tests run as one job per browser (Chromium and WebKit) for both Vite and the container. Each job owns its loopback emulator processes. The required `verify` status aggregates the complete reusable suite and fails if any required job fails, is cancelled, or is unexpectedly skipped; branch protection still requires it to be up to date. The image job builds the container once and exports it as a candidate; both container browser jobs load and test that exact image. A qualification job requires every job to succeed, validates the candidate from that same run against the exact source revision/tree and archive checksum, and publishes the **actual tested Linux/amd64 image**, its content ID, archive checksum, build commit, and full Git tree hash as a GitHub Actions artifact retained for 14 days. PR jobs have no Fly credentials.
...
3. Missing/expired images trigger the same three parallel verification jobs and qualification before release. The release job loads the qualified artifact from its own run; it never treats an in-progress external run or a candidate artifact as verified. A retained-image hit skips all three jobs and revalidates the existing image before release. If that artifact expires between lookup and loading, release fails closed. An API failure, invalid manifest, checksum mismatch, or image mismatch fails the release; it does not quietly downgrade to a cache miss. Images uploaded by failed/canceled/in-progress runs or forks are ineligible. Workflow, test, lockfile, rules and Fly-config changes all change the tree key.
...
PRs changing only `README.md`, `CHANGELOG.md`, or Markdown under `docs/` run lint/types/unit/server checks and skip browsers/containers. They produce **no deployable image**. Renames are classified as deletion plus addition so moving code into docs cannot skip tests. A subsequent deployment of a docs-only tree still needs a fully verified image and will run the full gate if none exists. The required `verify` aggregate always runs, avoiding permanently pending path-filtered branch checks. Docs-only classification explicitly requires successful checks and skipped browser jobs; any other combination fails.

Both workflows use `blacksmith-4vcpu-ubuntu-2404` with SHA-pinned external actions. Full runs cache Bun downloads, exact Playwright browsers and Firebase emulator binaries keyed by lockfile/toolchain; browser OS libraries are always installed. The pinned Blacksmith Docker builder uses its persistent layer cache and explicitly refuses setup fallback. Container verification uses `docker buildx build --load`, testing the resulting image before export. Caches accelerate work; they never stand in for a passing test result. The reusable `verification-suite.yml` isolates checks, Vite, and container jobs on separate Blacksmith machines. Each browser job runs one browser (`E2E_BROWSER=chromium|webkit`) with one worker, 60-second per-test limits, and one retry in CI; a test that passes only on retry is reported as flaky. Local runs keep zero retries and run both browsers unless `E2E_BROWSER` names one. No browser suite or coverage threshold is reduced. The image job builds once and transfers the image as a one-day candidate artifact; the container browser jobs validate and load that artifact (`artifacts.mjs load`) instead of rebuilding, and only the qualification gate publishes the 14-day verified artifact. Failed/cancelled overall workflow runs remain ineligible for later promotion, even if an artifact exists. Reports are retained independently as `verification-checks`, `verification-vite-<browser>`, and `verification-container-<browser>`.

A manual Actions run is available for `main` or `staging`. Deployments are serialized per branch with a 20-minute job timeout. Local verification has a ten-minute aggregate limit; the full release gate has twelve minutes because it runs the browser suite against both Vite and the container. Individual browser tests still have a 60-second timeout and one CI retry; each parallel lane has a ten-minute work budget and a twelve-minute job timeout, including setup. Qualification has five minutes and the required aggregate status has two. The local serial release command retains its twelve-minute cap; CI does not run both browser suites behind that single timer. The expanded 20-case suite exceeded the former ten-minute release budget during its final container case on PR #20, with the other 39 browser cases passing. Failures and reports live in GitHub Actions, independently of Fly; configure GitHub Actions failure notifications for the responsible maintainers. A failed deploy or smoke fails the job and requires operator attention, rather than automatically rolling back or claiming success.
...
Parallel-CI rehearsal is required before sign-off: exercise both a missing-image run (the three jobs must pass before any deployment) and a retained-image promotion through real Actions triggers, including an actual staging push. Verify the embedded revision through `/healthz` and HTTP smoke and record timings. New resource precondition: Blacksmith must be able to schedule three independent lanes; runner queues can reduce the wall-clock gain but cannot bypass the gate. Expected warm full verification is about 7–9 minutes instead of the serial suite exceeding 12 minutes; the image-hit path should remain about 1–3 minutes. These are estimates until the actual rehearsal is recorded.
