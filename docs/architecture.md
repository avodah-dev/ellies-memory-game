...
`track` records an immutable context reference, event props, sequence and monotonic/wall times in a 3000-entry drop-oldest ring. It does no serialization, IDB, UUID creation, scheduling or network work. Before startup it is inert except for this bounded buffer. A 2s timer drains at most 50 events per slice with a 4ms enrichment budget, yielding while work remains. Sink serialization/IDB scheduling also contributes to the reported `ms_drain`; JavaScript cannot preempt one expensive operation. Visibility-hidden/pagehide request a drain. Unsent events can be lost when a browser kills the page; this is not durable network delivery.
